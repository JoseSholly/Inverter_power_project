...
LOCMEM = {"default": {"BACKEND": "django.core.cache.backends.locmem.LocMemCache", "LOCATION": "api-cache-tests"}}
...
class ApplianceCachingEndpointTests(TestCase):
    ...
    def test_warm_list_and_calculations_make_no_queries(self):
        ...
        payloads = [
            ("/api/v1/power_calculator/calculate/", {"backup_time": 2, "battery_capacity": 200, "system_voltage": 12,
                                                     "solar_panel_watt": 300, "items": [{"id": self.fan.id}]}),
            ("/api/v2/power_calculator/calculate/", {"system_voltage": 12, "battery_capacity": 200, "solar_panel_watt": 300,
                                                     "items": [{"id": self.fan.id, "quantity": 1, "power_rating": 75,
                                                                "backup_time": 2}]}),
        ]
        with CaptureQueriesContext(connection) as queries:
            self.assertEqual(self.client.get(LIST_URL).status_code, 200)
            self.assertEqual(self.client.get("/api/v2/power_calculator/appliances/?name=fa").status_code, 200)
    ...
    def test_changes_show_up_immediately(self):
        ...
        self.fan.save()
        self.assertEqual([a["name"] for a in self.client.get(LIST_URL).json()], ["Ceiling Fan"])
        self.fan.delete()
    ...
    def test_calculation_sees_new_appliance_without_waiting(self):
        ...
        body = {"backup_time": 1, "battery_capacity": 200, "system_voltage": 12, "solar_panel_watt": 300,
                "items": [{"id": kettle.id}]}
    ...
    def test_etag_and_conditional_requests(self):
        ...
        for header in (etag, f"W/{etag}", f'"other", {etag}', "*"):
            with self.subTest(if_none_match=header):
                not_modified = self.client.get(LIST_URL, headers={"If-None-Match": header})
                self.assertEqual(not_modified.status_code, 304)
                self.assertEqual(not_modified.content, b"")
                self.assertEqual(not_modified.headers["etag"], etag)

        self.assertEqual(self.client.get(LIST_URL, headers={"If-None-Match": '"stale"'}).status_code, 200)
