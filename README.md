...
### Docker
A `Dockerfile` and `.dockerignore` at the repo root produce a self-contained image that runs `migrate`, seeds the default appliance catalogue with `populate_appliances` (idempotent — existing rows are skipped), then starts `runbolt` on `$PORT` (default `8000`). Static files are collected at build time and served by WhiteNoise.
...
4. The container seeds the appliance catalogue automatically on first boot; no manual command is needed.
5. To create an admin user without Render's Shell (unavailable on the free tier), point your local checkout at the production database and run `createsuperuser` from your machine — the Render Postgres dashboard provides an **External Database URL** you can use for this:
   ```bash
   cd inverter_project
